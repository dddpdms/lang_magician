...
model = load_facebook_vectors('../fasttext_model/model/sg300_emotion1_shoulder.bin')
f = open("data/교정실험_after_어깨번호/붙힌것/기본_세종1.txt", 'r', encoding='utf-8', newline='\n', errors='ignore')
...
while True:
    ...
    for i in split_list:
        if target_word in i:
            ...
            temp_str = split_list[cut_num].replace("본.00", "분.01")
            #split_list[cut_num] = split_list[cut_num].replace("본.00", "본.01")
